...
function kernel_downweighting(x_samples::Vector{Float64}, bandwidth::Float64, downweight_power::Float64; kernel_func::String = "Beta", sample_min::Union{Nothing, Float64} = nothing, sample_max::Union{Nothing, Float64} = nothing) 
    ...
    if kernel_func == "Beta"
        ...
        # map back to original interval [a,b] - this is just a linear scaling of the x-axis, so the density values should be unchanged since we will re-normalize anyway
        beta_kde_density = beta_kde_density .+ a ./ (b - a)
...
function std_dev_piecewise_gauss(f_pdf, x_grid, mean; nquad=5)
    ξ, w = gausslegendre(nquad)
    ...
    for j in 1:length(x_grid)-1
        ...
        integral += sum(wq .* (xq .- mean).^2 .* f_pdf.(xq))
    end
...
# Compute fit for truncated Gaussian given parameter samples 
function gaussian_fit(samples::Vector{ <: Real})
    n = length(samples)
    μ = sum(samples)/n 
    var = (1/n)*(sum((samples .- μ).^2))
    
    σ = sqrt(var)
    return Normal(μ, σ)
end

function downgrade_gaussian(samples::Vector{<: Real}, downgrade_power::Real)
    n = length(samples)
    μ = sum(samples)/n 
    var = (1/n)*(sum((samples .- μ).^2))
    σ = sqrt(var)
    σ_downgrade = σ*downgrade_power
    return Normal(μ, σ_downgrade)
end

function truncated_downgrade_gaussian(samples::Vector{<:Real}, downgrade_power::Real, lower::Real, upper::Real)
    n = length(samples)
    μ = sum(samples)/n 
    var = (1/n)*(sum((samples .- μ).^2))
    σ = sqrt(var)
    σ_downgrade = σ*downgrade_power
    D_trunc = truncated(Normal(μ, σ_downgrade), lower = lower, upper = upper)
    return D_trunc
end