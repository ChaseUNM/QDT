...
function log_prior_theta_w2_quantum_adaptive(
    ...
    λ_log_prior:: Union{Nothing, Distribution} = nothing
)   
...
function trace_wasserstein_squared_loss_quantum_multi(ω, ωr, degree, n_splines, U0, T, nsteps, pcof_optimal_total, event_obs_total, total_data_count, carrier_freqs; δ::Real, kwargs...)
...
function log_w2_posterior_quantum_adaptive(
    ...
    lp = log_prior_theta_w2_quantum_adaptive(ω, λ_log; ωmin = ωmin, ωmax = ωmax, λ_log_min = λ_log_min, λ_log_max = λ_log_max, ω_prior = ω_prior, λ_log_prior = λ_log_prior)
    if !isfinite(lp)
        return -Inf, Inf, -Inf, Inf
    end
...
function run_w2_chain_quantum(
    ...
    accept_theta_vec = zeros(initial_sample_length)
    chain_samples = zeros((iterations + 1)*initial_sample_length, 5)
    chain_kept_samples = length(collect(burnin:thin:iterations + 1))
    total_chain_kept_samples = zeros(initial_sample_length*chain_kept_samples, 5)
    diff_samples = zeros((iterations + 1)*initial_sample_length, 4)
    diagnostic_chain = zeros(chain_kept_samples, initial_sample_length)
    hyperparam_history = zeros(initial_sample_length, iterations + 1, 3)
...
function run_w2_chain_quantum_adaptive(
    ...
    initial_sample_length = length(ω0_vec)
    accept_theta_vec = zeros(initial_sample_length)
    accept_lambda_vec = zeros(initial_sample_length)
    chain_samples = zeros((iterations + 1)*initial_sample_length, 6)
    chain_kept_samples = length(collect(burnin:thin:iterations + 1))
    total_chain_kept_samples = zeros(initial_sample_length*chain_kept_samples, 6)
    diff_samples = zeros((iterations + 1)*initial_sample_length, 4)
    diagnostic_chain = zeros(chain_kept_samples, initial_sample_length)
...
function run_w2_chain_quantum_sum(
    event_obs_total;
    prior::Union{Nothing, NamedTuple, Distribution} = nothing,
    ω0_vec::Vector{<:Real} = zeros(length(ω0_vec)), 
    ωr::Real = 0.0, 
    ...
    for ω0 in ω0_vec
        chain = zeros(iterations + 1, 5)
        diff_chain = zeros(iterations, 4)
        logpost0, Φ0, Φ0_scaled, logprior0 = log_w2_posterior_quantum_sum(
            ω0, 
            ωr, 
            degree, 
            n_splines, 
        ...
        for iter in 1:iterations
            rand_number = randn(rng)
            ω_prop = ω_curr + exp(ηθ) * sqrt(Σθ) * rand_number
            logpost_prop, Φ_prop, Φ_prop_scaled, logprior_prop = log_w2_posterior_quantum_sum(
                ω_prop, 
                ωr, 
                degree, 
                n_splines, 
